// Where: cli/internal/generator/merge_config_test.go
...
func TestMergeFunctionsYml(t *testing.T) {
	...
	scaling := asMap(defaults["scaling"])
...
func TestMergeResourcesYml(t *testing.T) {
	...
	resources := asMap(result["resources"])

	// DynamoDB tables
	dynamo := asSlice(resources["dynamodb"])
	if len(dynamo) != 2 {
