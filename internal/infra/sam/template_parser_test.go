// Where: cli/internal/infra/sam/template_parser_test.go
...
func findFunction(fns []template.FunctionSpec, name string) *template.FunctionSpec {
	for _, fn := range fns {
		if fn.Name == name {
			return &fn
		}
...
`
	...
	find := func(name string) *template.FunctionSpec {
