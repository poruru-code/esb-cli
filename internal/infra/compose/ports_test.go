// Where: cli/internal/infra/compose/ports_test.go
...
func TestDiscoverPortsFiltersByMode(t *testing.T) {
	...
	for _, name := range []string{
		...
		if err := os.WriteFile(filepath.Join(rootDir, name), []byte("test"), 0o600); err != nil {
	...
	ports, err := DiscoverPorts(t.Context(), runner, PortDiscoveryOptions{
		RootDir: rootDir,
		Project: "esb-test",
		Mode:    "docker",
		Target:  "control",
