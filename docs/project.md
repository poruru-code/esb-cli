...
`esb project` コマンドファミリは、異なるESBプロジェクトの登録、移動、および管理に使用されます。プロジェクトのグローバルレジストリ（`~/.esb/config.yaml`、`ESB_CONFIG_PATH`/`ESB_CONFIG_HOME` で上書き可能）を維持し、ファイルシステムを移動することなくコンテキストを簡単に切り替えることができます。
